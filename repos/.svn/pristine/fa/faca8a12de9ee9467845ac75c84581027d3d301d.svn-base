 ...
 */
@Repository
public class SendSmsQueueDAO extends HibernateCommonDAO<TbSendSmsQueue>{

	private static final Logger LOGGER = LoggerFactory.getLogger(SendSmsQueueDAO.class);
	...
	* @since mms-task
	 */
	public void saveBatchSendSms(String insertSql,List<TbSendSmsQueue> smsList){
		Session session = this.getSession();
		try {
			session.beginTransaction(); //开启事物
			for (int i = 0; i < smsList.size(); i++) {
				TbSendSmsQueue sendSms = smsList.get(i);
