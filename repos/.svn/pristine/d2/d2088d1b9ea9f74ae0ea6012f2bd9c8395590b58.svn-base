	...
	var griduser=Ext.create('Ext.grid.Panel', {
		...
		columns: [
		          ...
		          { header: '操作', dataIndex: '',renderer:function(value, meta, rec, rowIdx, colIdx, ds){
		        	  ...
		        	  +'</a>|<a href="#" onclick="deleteOneType(\''+rec.get('id')+'\')">'+"删除"
		        	  +'</a>';
		          }}
		          ],
		          bbar : new Ext.PagingToolbar({   //添加翻页工具条
		        	  store : decorationTypeStore,   //将数据源添加到分页的工具条中
...
//		提交按钮
		var submitButton = new Ext.Button({
			...
			listeners:{
				click:function(){
					saveDecorationTypeInfo(serviceInfoPanel);
				}
			}
		});
//		取消按钮
		...
		var serviceInfoPanel = Ext.create('Ext.form.FormPanel', {
			...
			items: [
			        ...
			        {xtype:"textfield",fieldLabel:"饰物分类名称",id:'name',width:270,allowBlank: false,maxLength:20},
			        ],
