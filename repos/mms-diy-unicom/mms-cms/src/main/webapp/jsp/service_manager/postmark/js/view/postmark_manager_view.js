Ext.onReady(function() {
	...
	var griduser=Ext.create('Ext.grid.Panel', {
		...
		columns: [
		          ...
		          { header: '邮戳名称',  dataIndex: 'name' ,flex:1},
		          ...
		          { header: '操作', dataIndex: '',renderer:function(value, meta, rec, rowIdx, colIdx, ds){
		        	  return '<a href="#" onclick="modify(\''+rec.get('id')+'\')">修改</a>|<a href="#" onclick="deleteOnePostmark(\''+rec.get('id')+'\')">删除</a>';
		          }}
		          ],
	      bbar : new Ext.PagingToolbar({   //添加翻页工具条
        	  store : postMarkStore,   //将数据源添加到分页的工具条中
	...
	var submitButton = new Ext.Button({
		...
		listeners:{
			click:function(){
				...
				var pkid = Ext.getCmp('postmarkPkid').getValue();
				var path = "";
				if(pkid==null || pkid==""){
					path = ctx+'/postmark/postmark/postmark!savePostmark.action'
				}else{
					path = ctx+'/postmark/postmark/postmark!updatePostmark.action'
				}
	...
	var formUpload = new Ext.form.FormPanel({
		...
		items: [
		        {xtype:"textfield",fieldLabel:"主键pkid",name:'model.id',id:'postmarkPkid',width:270,maxLength:20,hidden:true},
		        {xtype:"textfield",fieldLabel:"邮戳名称",name:'model.name',id:'postmarkName',width:270,allowBlank: false,maxLength:20},
		        statusCombox, 
