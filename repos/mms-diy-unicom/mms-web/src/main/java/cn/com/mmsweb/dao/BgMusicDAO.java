...
public class BgMusicDAO extends CommonDAO<TbBgMusic> {

	private static final Logger LOGGER = Logger.getLogger(BgMusicDAO.class);
	
	public List<PhotoFrameVo> queryByPage (int pageSize, int pageNow,int groupsId) {
		   List<TbBgMusic> list = new ArrayList<TbBgMusic>();
		   ...
		   try {
			   ...
			   sb.append(" from TbBgMusic c ");
			   sb.append(" order by c.id ");
			   Page<TbBgMusic> page = new Page<TbBgMusic>();
			   page.setPageSize(pageSize);
			   page.setPageNo(pageNow);
		       Page<TbBgMusic> pagelist=this.findByPage(page, sb.toString());
		       list = pagelist.getResult();
		       for(TbBgMusic bgMusic:list){
		    	   PhotoFrameVo groupContactsVo=new PhotoFrameVo();
		    	   groupContactsVo.setId(bgMusic.getId());
		    	   groupContactsVo.setName(bgMusic.getName());
		    	   groupContactsVo.setPicUrl(bgMusic.getFileUrl());
