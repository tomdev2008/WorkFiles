...
public class TbDataSourceEntityController extends BaseController {
	...
	@RequestMapping(params = "getDataSourceByType")
	@ResponseBody
	public List getDataSourceByType(HttpServletRequest request) {
		
		String id = request.getParameter("id");
		String hql = "from TbDataSourceEntityEntity where 1 = 1 AND tbDataSourceType.id = ? ";
		List<TbDataSourceEntityEntity> tbDataSourceList = systemService.findHql(hql,id);
		System.out.println("---------"+tbDataSourceList.size());
		return tbDataSourceList;
	}
}
