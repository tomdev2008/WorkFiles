  	...
  	function queryContractInfoById() {
  		...
  		$.ajax({
	    	...
	    	success:function(data) {
				if(data!=null){
					$("#amount").val(data.attributes.contractmoney);
				}
	    	}
	    });
  	}
    ...
    function checkAddOrUpdate() {
  	  var endid = '${tbInvoicePage.id }';
  		if(null != endid && "" != endid) {
  			...
  			var status = '${tbInvoicePage.status}';
  			$("#contractId_id").val(contractId);
  			$("#taxType").val(taxType);
  			$("#purchaseCompanyId").val(purchaseCompanyId);
  			$("#customerId_id").val(customerId);
  			$("#status").val(status);
  		}
    }
  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tbInvoiceController.do?save">
			...
			<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
				<tr>
					...
					<td class="value">
						<select id="contractId_id"  name="contractId.id" datatype="*" onchange="queryContractInfoById();">
				...
				<tr>
					...
					<td class="value">
						<select id="customerId_id"  name="customerId.id" datatype="*">
