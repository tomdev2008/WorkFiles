 ...
 */
public class VmHostCore extends MapsCore {

	private static final Logger LOGGER = Logger.getLogger(VmHostCore.class);
	...
	* @since system_center_sdk
	 */
	public static GetVmHost getVmHost(){
		return new GetVmHost();
	}
	
	//获取所有硬件配置信息
	public static class GetVmHost implements Command<Maps>{
		...
		public Maps execute(String clientType) throws Exception {
			try {
				...
				if(BusinessConstants.TYPE_SYSTEMCENTER.equals(clientType)){
					...
					hardwareProfileCommand.append("Get-SCVMHost -VMMServer \""+vmmServer+"\"");
