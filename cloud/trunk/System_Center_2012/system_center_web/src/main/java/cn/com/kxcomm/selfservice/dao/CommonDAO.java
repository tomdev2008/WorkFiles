...
import cn.com.kxcomm.common.ReflectionUtils;
...
public class CommonDAO<T> extends  HibernateDaoSupport {  
	...
	public void save(T entity) {  
		try {  
			...
			if (logger.isDebugEnabled()) {  
				logger.debug("保存实体类成劄1�7," + entity.getClass().getName());  
			}  
	...
	public void delete(T entity) {  
		try {  
			...
			if (logger.isDebugEnabled()) {  
				logger.debug("删除实体类成劄1�7," + entity.getClass().getName());  
			}  
		} catch (RuntimeException e) {  
	...
	public void deleteAll(Collection<T> entities) {  
		try {  
			...
			if (logger.isDebugEnabled()) {  
				logger.debug("删除实体类成劄1�7," + entities);  
			}  
		} catch (RuntimeException e) {  
			logger.error("删除实体异常", e);  
			throw e;  
	...
	public List<T> findAll() {  
		try {  
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始删除实体：" + entityClass.getName());  
			}  
	...
	public void saveOrUpdate(T entity) {  
		try {  
			...
			if (logger.isDebugEnabled()) {  
				logger.debug("更新或�1�7�保存实体成劄1�7," + entity.getClass().getName());  
			}  
		} catch (RuntimeException e) {  
			logger.error("更新或保存实体异帄1�7", e);  
			throw e;  
		}  
	}    
	...
	public T findById(Serializable id) {  
		try {  
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始查找ID丄1�7" + id + "的实体：" + entityClass.getName());  
			}  
			return (T) getHibernateTemplate().get(entityClass, id);  
		} catch (RuntimeException e) {  
			logger.error("查找指定ID实体异常，ID＄1�7" + id, e);  
			throw e;  
		}  
	}
	...
	public List find(String hql, Object... values) {  
		try {  
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始查询指定HQL语句," + hql);  
			}  
			return getHibernateTemplate().find(hql, values);  
		} catch (RuntimeException e) {  
			logger.error("查询指定HQL异常，HQL＄1�7" + hql, e);  
			throw e;  
		}  
	}  	
	...
	public Object findUnique(final String hql, final Object... values) {  
		try {  
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始查询返回唯丄1�7结果的HQL语句," + hql);  
			}  
	...
	/**
	 * 
	 * 根据查询条件获取符合条件的数据条敄1�7
	 * @author chenxinwei 新增日期＄1�712-1-22
	 * @param criteria
	 * @return
	 */
	...
	public List<T> findByProperty(Class<T> entityClass, String propertyName,  
		...
		try {  
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始查找指定属性：" + propertyName + "丄1�7" + value + "的实佄1�7"  
						+ entityClass.getName());  
			}  
	...
	/** 
	 * 模糊查询指定条件对象集合 <br> 
	 * 用法：可以实例化丄1�7个空的T对象，需要查询某个字段，就set该字段的条件然后调用本方泄1�7br> 
	...
	* @param hql
	* @param p
	* @author chenliang 新增日期＄1�712-5-18
	* @author 你的姓名 修改日期＄1�712-5-18
	* @since gdgroup
	 */
	...
	public Page<T> findByPage(final Page<T> page, final String hql, 
		...
		try {  
			...
			if (logger.isDebugEnabled()) {  
				logger.debug("弄1�7始查找指定HQL分页数据," + hql);  
			}  
			return (Page<T>) getHibernateTemplate().execute(  
					new HibernateCallback() {  
						...
						throws HibernateException, SQLException {  
							...
							//总页数
							int totalCount = findTotalCount(counthql, values); 
							//最大页数
							int maxPage = totalCount%page.getPageSize()>0?totalCount/page.getPageSize()+1:totalCount/page.getPageSize(); 
			...
			logger.error("分页查询异常，HQL＄1�7" + hql, e);  
			throw e;  
		}  
	}  
	 ...
	 * @return
	 */
	public Page<T> findByPage(Example example, Page<T> pageInfo){
		DetachedCriteria criteria = DetachedCriteria.forClass(entityClass);
		criteria.add(example);
		return this.findByPage(criteria, pageInfo);
	}
	
	public Page<T> findByPage(Page<T> pageInfo){
		DetachedCriteria criteria = DetachedCriteria.forClass(entityClass);
		return this.findByPage(criteria, pageInfo);
	}
	...
	public Page<T> findByPage(DetachedCriteria criteria, Page<T> pageInfo){
		...
		pList.setResult(this.findByCriteria(criteria, pageInfo.getStart(), pageInfo.getPageSize()));
		return pList;
	}
	...
	/**
	 * 根据某字殄1�7某�1�7�查询单个对豄1�7
	 * @author wukq 新增日期:2012-03-23
	 * @param fieldName 字段各1�7
	 * @param fieldValue 字段倄1�7
	 * @return 单个对象 戄1�7 null
	 */
