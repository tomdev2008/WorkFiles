...
public class Sensor {
	 ...
	 * @since ipmi_sdk
	 */
	public List<GlobalEntity> getSensorInfo(SerInfo ser,String name){
	...
	public List<List> getSensorListInfo(SerInfo ser,String type){
		List<List> list = new ArrayList<List>();
		List<String> ll = sdr.getSdrList(ser,type);
		for(int i=0;i<ll.size();i++){
			String name = ll.get(i);
			List<GlobalEntity> cc = getSensorInfo(ser, name);
			if(cc.size()>0){
				list.add(cc);
			}
		}
	 ...
	 * @since ipmi_sdk
	 */
	public List<List<GlobalEntity>> getFanRPMInfo(SerInfo ser){
		List<String> list = getFanRPM(ser);
		List<List<GlobalEntity>> ll  = new ArrayList<List<GlobalEntity>>();
		for(String str:list){
			List<GlobalEntity> cc = getSensorInfo(ser, str);
			ll.add(cc);
		}
		return ll;
	}
	 ...
	 * @since ipmi_sdk
	 */
	public List<List<GlobalEntity>> getCPUInfo(SerInfo ser){
	 ...
	 * @since ipmi_sdk
	 */
	public List<List<GlobalEntity>> getBatteryInfo(SerInfo ser){
	 ...
	 * @since ipmi_task
	 */
	public Integer getTempSensorReading(SerInfo ser,String tempType){
		if(!StringUtils.equals(tempType, Parameter.InletTemp)&&!StringUtils.equals(tempType, Parameter.ExhaustTemp))
			return null;
		int reading = 0;
		List<GlobalEntity> temp = getSensorInfo(ser, tempType);
