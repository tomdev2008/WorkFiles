...
<div class="easyui-layout" fit="true">
  <div region="center" style="padding:1px;">
		<t:datagrid name="tbCheckingInstanceList" actionUrl="tbCheckingInstanceController.do?datagrid" idField="id" fit="true" queryMode="group" onClick="operationDetail">
...
<%-- 	   <t:dgToolBar title="编辑" icon="icon-edit" url="tbCheckingInstanceController.do?addorupdate" funname="update"></t:dgToolBar> --%>
	   <t:dgToolBar title="查看" icon="icon-search" url="tbCheckingInstanceController.do?addorupdate" funname="detail"></t:dgToolBar>
	    ...
	    <t:dgToolBar title="删除" icon="icon-remove" url="tbCheckingInstanceController.do?del" funname="delone"></t:dgToolBar>
	  </t:datagrid>
  </div>
...
<script type="text/javascript">
	...
	<%--function operationDetail(functionId) {
	 ...
	 function delone(title,url, id) {
		 	...
		 	url += '&id='+rowData.id+'&empId_id='+rowData.empId_id+'&empName='+rowData.empName+'&happenday='+rowData.happenday;
		 	delObj(url,id);	 	
	}
	//展现出考勤明细
	 function operationDetail(rowIndex, rowData)
	 {
		var st = $("input[name='happenday_begin']").val();
		var et = $("input[name='happenday_end']").val();
		$('#operationDetailpanel').panel("refresh", "tbCheckingInstanceController.do?queryInfo&empId_id="+rowData.empId_id+"&happenday_begin="+st+"&happenday_end="+et);
	 }
</script>