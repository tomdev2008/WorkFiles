...
//		codeParamEntityIn.setTableName("tb_orders_main");//主表[表名]
//		codeParamEntityIn.setEntityName("TbOrdersMain");	 //主表[实体名]
//		codeParamEntityIn.setEntityPackage("orders");	 //主表[包名]
//		codeParamEntityIn.setFtlDescription("订单表");	 //主表[描述]
//		
//		//第二步：设置子表集合
//		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
//		//[1].子表一
//		SubTableEntity po = new SubTableEntity();
//		po.setTableName("tb_details_dom");//子表[表名]
//		po.setEntityName("TbDetailsDom");	 //子表[实体名]
//		po.setEntityPackage("orders");			 //子表[包]
//		po.setFtlDescription("订单详情表");		 //子表[描述]
//		//子表[外键:与主表关联外键]
...
//		CodeGenerateOneToMany.oneToManyCreate(subTabParamIn, codeParamEntityIn);
	...
	//--------------------------------------------------------
		//第一步：设置主表
		CodeParamEntity codeParamEntityIn = new CodeParamEntity();
		codeParamEntityIn.setTableName("tb_purchase_contract");//主表[表名]
		codeParamEntityIn.setEntityName("TbPurchaseContract");	 //主表[实体名]
		codeParamEntityIn.setEntityPackage("contactm");	 //主表[包名]
		codeParamEntityIn.setFtlDescription("采购合同");	 //主表[描述]
		
		//第二步：设置子表集合
		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
		//[1].子表一
		SubTableEntity po = new SubTableEntity();
		po.setTableName("tb_purchase_copy");//子表[表名]
		po.setEntityName("TbPurchaseCopy");	 //子表[实体名]
		po.setEntityPackage("contactm");			 //子表[包]
		po.setFtlDescription("采购订单");		 //子表[描述]
		
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系
		po.setForeignKeys(new String[]{"purchase_contract_id"});
		subTabParamIn.add(po);
