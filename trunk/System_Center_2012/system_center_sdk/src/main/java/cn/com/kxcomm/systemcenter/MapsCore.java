 ...
 */
public class MapsCore extends Core{
	
	private static final Logger LOGGER = Logger.getLogger(MapsCore.class);
	...
	* @since system_center_sdk
	 */
	protected static Maps setMaps(Process ps) throws IOException {
		BufferedReader rd = new BufferedReader(new InputStreamReader(ps.getInputStream(),"GB2312"));
		List<Hash> mapEntityList = new ArrayList<Hash>();
	...
	* @since system_center_sdk
	 */
	protected static Maps executeScript(String scriptStr,String output) throws IOException {
		try {
			//1、创建powershell脚本文件
			//1.1创建执行文件，设置验证变量
