...
public class CmsUserServiceImpl extends CommonServiceImpl<TbCmsUser> implements CmsUserService {
	public static Logger log = Logger.getLogger(CmsUserServiceImpl.class);
	
	@Autowired(required = true)
	public CmsUserDAO cmsUserDAO;
...
//		}
		...
		return cmsUserDAO.findByPage(pageInfo, hql.toString(), entity.getAccouont(),entity.getUserName());
	}
	/**
	 * 旧密码验证
	 * 
	...
	public TbRole queryConsumerRole(Long consumerId) {
		...
		if(!BlankUtil.isBlank(consumerId)){
			TbCmsUser userEntity = cmsUserDAO.findById(consumerId);
			if(!BlankUtil.isBlank(userEntity)){
...
////          方案1:经测试，查询时间大概范围为：15ms--47ms			
			...
			conn = SessionFactoryUtils.getDataSource(this.cmsUserDAO.getSessionFactory()).getConnection();;// 获得连接
