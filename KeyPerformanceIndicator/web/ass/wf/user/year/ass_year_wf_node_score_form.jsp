...
<body class="LN_body">
<form name="form1" method="post" action="${ROOT }/saveYearAssessmentIndicatorsScoreByUser.do?workitemId=${param.workitemId}">
	          ...
	          <!-- 取值渠道，在考核表中不显示 -->
	          	...
	          	<c:choose>
	          			<c:when test="${o.showExtType==1}">
	          				<td width="" class="LN_tableBoxtitle2" colIndex="${o.code}" style="display:${showNone};word-break : break-all;">
	          				${o.name }
	          				<input type="checkbox" id="selectAllHandler1" onclick="SelectUtil.checkAll(this,'priorityCBX');"/>
	          				</td>
	          			</c:when>
	          			<c:when test="${o.showExtType==4}">
	          				<td width="3%" class="LN_tableBoxtitle2" colIndex="${o.code}" style="display:${showNone};word-break : break-all;text-align: center;">
	          					考核项目
	          				</td>
	          			</c:when>
