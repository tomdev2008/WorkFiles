...
<!--
	function ev_showAssessment(assID,userId,isUndo){
		...
		var params = "?boId="+assID
			...
			+"&viewType=1&isUndo="+isUndo+"&srcHandlerId="+userId;
		
			url = "${ROOT}/editYearAssessmentAllByUser.do";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="LN_infbox-border">
  ...
  <tr>
    <td class="LN_press">
            ...
            <c:forEach var="o" items="${assessmentList}" varStatus="idx">
	            <tr>
	              ...
	              ${idx.count}<input type="hidden" name="assId" value="${o.id }"/>
	            	...
	            	<td class="LN_tableBoxText2">
	            	<a href="#" class="LN_BlueLink" onclick="ev_showAssessment('${o.id }','${boInfoM[o.id].srcHandlerId}',${boInfoM[o.id].status==1});return false;">${o.ownerName }</a></td>
		            ...
		            <td class="LN_tableBoxText2">
						<c:forEach var="o2" items="${o.assNodeGradeHandlerSums}" varStatus="idx1">
							...
							<c:if test="${o1.id==o2.nodeId}">
								评分${idx1.count }：${empty o2.score?"未评分":"已评分" }<br/>
							</c:if>
						</c:forEach>
					</td>
				</c:forEach>
				<td class="LN_tableBoxText2">
					<c:forEach var="o2" items="${o.assNodeGradeHandlerSums}" varStatus="idx1">
							<c:if test="${lastNode.id==o2.nodeId}">
								${empty o2.score ?"未评分":"已评分"}<br/>
							</c:if>
	
						</c:forEach>
				</td>
				<c:if test="${fn:length(o.assNodeGradeHandlerSums)<1}">
					<td class="LN_tableBoxText2">
	            		<fmt:formatNumber value="${o.middleScore}" pattern="##0.0#"/>&nbsp;
	            	</td>
	            	<td class="LN_tableBoxText2">
	            		<fmt:formatNumber value="${o.middleScore}" pattern="##0.0#"/>&nbsp;
	            	</td>
		  ...
		  <tr>
		     <td colspan="8" class="LN_tableBoxtitle">
		     	<div align="center">
		     	说明：员工互评分40%+部门经理评分60%=考核评分
		     	</div>
		     </td>
		  </tr>
      </td>
