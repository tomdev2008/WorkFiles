...
<head>
   <%
    ...
    <script>
       ...
       String.prototype.trim = function()
       {
        ...
        {  
     	...
     	//打开月度考核平均得分，查看所一周内的月度考核得分
	    function ev_showMonthScore(gradeId,assId,ownerId,batchDate){
			var url = "${ROOT}/editAvgMonthScore.do?gradeId="+gradeId+"&assId="+assId+"&ownerId="+ownerId+"&isUndo=false&batchId=${batchId}&batchDate="+batchDate;
    	...
    	function query()
    	{
    	   ...
    	   document.form1.action="<%=request.getContextPath()%>/queryYearAssessment.do";
    ...
    function AA_Show(id){
		var params = "?boId="+id+"&viewType=2&isUndo=false";
...
<form name="form1" id="form1" method="post" action="${ROOT}/queryYearAssessment.do">
<div class="LN_formTitle2">
    ...
    <!--<c:if test="${!empty list}">
    	 <div class="LN_TopInfor" style="background-color:white">
          ...
          --><table width="100%" border="0" cellspacing="1" cellpadding="0" class="LN_tableBox2">
            ...
            <td  class="LN_tableBoxtitle2" style="text-align:center;">月度考核平均分</td>
            <td class="LN_tableBoxtitle2" style="text-align:center;">行为考核成绩</td>
            <td  class="LN_tableBoxtitle2" style="text-align:center;">部门成绩</td>
            <td  class="LN_tableBoxtitle2" style="text-align:center;">汇总得分</td>
            <td  class="LN_tableBoxtitle2" style="text-align:center;">评分等级</td>
            <td  class="LN_tableBoxtitle2" style="text-align:center;">考核周期</td>
            </tr>
             ...
             <c:choose>
               <c:when test="${!empty list}" >
                  ...
                  <tr id="showTr_${o.assessment.id}" name="qryOutTR" >
                  <td class="LN_tableBoxText2">${idx.count}</td>  
                  <td class="LN_tableBoxText2" >&nbsp;${o.orgName==null?o.assessment.owner.parentName:o.orgName}</td>
                  <td class="LN_tableBoxText2" >&nbsp;${o.assessment.ownerName}</td>
                  <td class="LN_tableBoxText2" >
                  	<a href="#" class="LN_BlueLink" title="查看月度考核成绩" onclick="ev_showMonthScore('${o.id }','${o.assessment.id }','${o.assessment.ownerId }','${o.assessment.batchDate }')">
	            		${o.avgMonthScore}
	            	</a></td>
                  <td class="LN_tableBoxText2" >${o.yearScore}</td>
                  <td class="LN_tableBoxText2" >${o.orgScore}</td>
                  <td class="LN_tableBoxText2" >
                  <a href="#" class="LN_BlueLink" title="查看考核表" onclick="AA_Show('${o.assessment.id}')">
                  	<fmt:formatNumber value="${o.assessment.assScore}" pattern="##0.0#"/>
                  </a>
                  </td>
                  <td class="LN_tableBoxText2">
	            	${o.assGrade!=null?o.assGrade:o.assGradeValue}
	              </td>
	              <td class="LN_tableBoxText2">
	              	${o.assessment.batchDateDesc }
	              </td>
                  </tr>
                  </c:forEach>	
