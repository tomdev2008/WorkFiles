...
<%
   ...
   String ifilename="组织月度考核成绩查询.xls"; 
   if(selectType.equals("quarter")){
      ifilename="组织季度考核成绩查询.xls"; 
   }
   if(selectType.equals("year")){
      ifilename="组织年度考核成绩查询.xls"; 
   }
...
<body class="LN_body" onmousewheel="setScrollzheight()">
        	...
        	<tbody>
        		<tr>  
        			<td  colspan="9" align="center" height="32" style='width:1200px;'>
        				<h4><c:if test="${selectType eq 'month' }">组 织 月 度 考 核 查 询</c:if>
        				<c:if test="${selectType eq 'quarter' }">组 织 季 度 考 核 查 询</c:if>
        				<c:if test="${selectType eq 'year' }">组 织 年 度 考 核 查 询</c:if></h4>
        			</td>
      ...
      <form name="form1" method="post" action="">
        ...
        <c:if test="${singleQry eq 'false' }">
         <tr><td style="text-align:left;">部门：&nbsp;${orgName}</td></tr>
	     <tr><td style="text-align:left;">查询时间范围：&nbsp;${selYear}年</td></tr>
        </c:if>
            ...
            <td width="10%" class="LN_tableBoxtitle2" style="text-align:center;">评分周期</td>
             ...
             <c:choose>
               <c:when test="${!empty list}" >
                  ...
                  <td class="LN_tableBoxText2" >${o.endOrgName}</td>
