...
<html>
	<head>
		...
		<script language="javascript">
			...
			function reloadAll(id, clear){
				var entryClass = document.getElementById("class_"+id).value;
				window.location.href = "${ROOT}/reloadCache.do?class="+entryClass+"&clear="+clear;
			}
			
			function testSpring(){
				var tempCount = document.getElementById("tempCount").value;
				window.location.href = "${ROOT}/testSpring.do?tempCount="+tempCount;
			}
		</script>
	</head>
		...
		<form action="" method="post" >
			...
			<%
			 ...
			 %>
				<tr>
					<td width="5%"><%=i%></td>
					<td width="35%" align="left"><%=name+"["+entryClass.getName()+"]"%></td>
					<td width="15%"><%=size%></td>
					<td width="15%"><input type="text" id="id_<%=i%>" /> </td>
					<td width="20%">
						<input type="hidden" id="class_<%=i%>" value="<%=entryClass.getName()%>" />
						<input type="button" value="清空" onclick="eveict('<%=i%>')" />
						<input type="button" value="再次载入" onclick="reloadAll('<%=i%>', '0')" />
						<%--<input type="button" value="重载" onclick="reloadAll('<%=i%>', '1')" /> 
					--%></td>
				</tr>
			<%
				}}
			...
			<a href="javascript:testSpring();">创建</a><input type="text" id="tempCount" name="tempCount" />个临时对象。
		</form>
	</body>
</html>