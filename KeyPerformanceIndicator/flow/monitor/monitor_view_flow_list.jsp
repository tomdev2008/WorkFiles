...
<body onmousewheel='setScrollzheight();' class="LN_body">
    <form name="form1" method="post" style="margin-top: 0px">
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="list-top">
        	<tbody>
        		<tr>  
        			<td><h2><c:if test="${busiId=='YGKH'}">员工</c:if><c:if test="${busiId=='BMKH' }">组织</c:if>监控管理</h2></td>
        		</tr>
        ...
        <table class="list-box" cellspacing="0" border="0">					
			<tr>
				<td class="list-title" width="8%" align="right">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;序号</td>
				<td class="list-title" width="20%">部门</td>
				<td class="list-title" width="44%">名称</td>
				<td class="list-title" width="18%">类别</td>
				<td class="list-title" width="10%" colspan="2">操作</td>
			</tr>
		<c:forEach var="flow" items="${flowList}" varStatus="idx">
			<tr class="list-color${idx.index%2==0?'1':'2'}">
				...
				<td class="list-text1">${flow.org.name}</td>
				<td class="list-text1">
					<kpi:secure name="${flow.flowName}" code="${typeCode }200" resourceId="${orgId }" type="viewLink" title="查看流程定义" />
				</td>
