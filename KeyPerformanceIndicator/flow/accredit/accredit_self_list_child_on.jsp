   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要删除记录？")) {
	     ...
	     accreditForm.action = "${ROOT}/deleteSelfAccredit.do?action=listSelfOnAccredit.do";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要启动该授权？")) {
      	 accreditForm.target="_self";
	     accreditForm.action = "${ROOT}/onSelfAccredit.do?action=listSelfOnAccredit.do";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
   ...
   var selected=new Array();
    ...
    if (count >0)  {
      if (window.confirm("您确认要停止该授权？")) {
      	 accreditForm.target="_self";
	     accreditForm.action = "${ROOT}/offSelfAccredit.do?action=listSelfOnAccredit.do";
         accreditForm.submit();
      }
    }else {
       alert("您还没选择记录!");
...
function ev_see(id)
{
	var url="${ROOT}/listWorkListItemByAccreditOn.do?acid="+id;  
	window.open(url,'','left=100, top=80, width=800, height=600, resizable=1,scrollbars=1');
}
 </script>
</head>
