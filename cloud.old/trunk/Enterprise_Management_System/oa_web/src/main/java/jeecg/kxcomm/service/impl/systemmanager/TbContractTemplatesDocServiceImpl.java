...
public class TbContractTemplatesDocServiceImpl extends CommonServiceImpl implements TbContractTemplatesDocServiceI {
	...
	//获取合同模板关联的合同模板文件
	@Override
	public List<TbContractTemplatesDocEntity> getConTempList(String temple_id) {
	...
	/*	hqlQuery.setQueryString(hql.toString());
		...
		System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+temple_id);
		SQLQuery query = getSession().createSQLQuery(hqlQuery.getQueryString());
	 	List list= query.list();
	 	System.out.println("!!!!!!!!!!!!!!!!!!"+list.size());*/
		List<Map<String, Object>> list=this.findForJdbc(hql.toString());
	    	System.out.println("@@@@@@@@@@@@@@@@@@"+list.size());
		...
		for (int i = 0; i < list.size(); i++) {
			...
			vo.setDocname((String)map.get("docName"));
			System.out.println("@@@@"+(Date)map.get("createtime"));
			try {
			...
			System.out.println("@@@@@@@@@@@@@@@"+(String)map.get("docName"));
			vo.setBvariable((Integer)map.get("bVariable"));
			volist.add(vo);
		}
		return volist;
	}
	...
	//获取合同模板关联的合同模板文件
	@Override
	public PageList getConTempDocList(HqlQuery hqlQuery, boolean b,TbContractTemplatesDocEntity tbContractTemplatesDocEntity,String temple_id) {
		//条件拼接，根据登陆不同的角色，看到的订单不同
		StringBuffer whereSql = new StringBuffer();
		if(null != temple_id && !("").equals(temple_id)) {
			whereSql.append(" and c.id = '"+temple_id+"' ");
