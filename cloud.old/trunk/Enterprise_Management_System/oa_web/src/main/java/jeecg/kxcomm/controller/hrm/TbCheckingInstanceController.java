...
//		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tbEmployee);
	...
	* @since oa_web
	...
	public  AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				tbCheckingInstanceService.importExcel(file.getInputStream(),startTime,endTime);
				j.setMsg("文件导入成功！");
			} catch (IOException e) {
				j.setMsg("文件导入失败！");
	...
	* @since oa_web
	 */
	@RequestMapping(params="staticts")
	@ResponseBody
	public  AjaxJson staticts(HttpServletRequest request, HttpServletResponse response) throws Exception{
		AjaxJson j = new AjaxJson();
		
		String startTime = "";
		String endTime = "";
		
		tbCheckingInstanceService.statisticsChecking(startTime,endTime);
