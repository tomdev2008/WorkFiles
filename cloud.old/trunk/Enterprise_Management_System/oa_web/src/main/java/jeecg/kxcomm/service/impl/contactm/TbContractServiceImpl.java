...
/*	@Override
	...
	public PageList getPageList(HqlQuery hqlQuery, boolean b,
		...
		hql.append(" SELECT SUM(b.receive_money) as bsum,tt.ct,tt.aname,tt.cname,tt.adata,tt.ano,tt.anumber,tt.aremark,tt.aid,tt.astatus,tt.uname "); 
		hql.append(" FROM (SELECT a.contract_no ano,a.contract_number anumber,a.contract_date adata,a.remark aremark,"+
				"a.contract_total_price ct,a.id aid,a.contract_name aname,c.company_name cname,a.status astatus,base.realname uname");
...
/*	@Override
	public PageList getConfigByQuoList(HqlQuery hqlQuery, boolean b,
		...
		sql.append(" SELECT b.id as bid,a.id as aid,a.total_price as total,c.source_id,c.sourceName,a.name ");
		sql.append(" from tb_config_models a,tb_quotations b,tb_data_source c ");
		sql.append(" WHERE a.quotations_id=b.id AND c.source_id=a.data_source_id ");
		sql.append(" and b.id = '"+quoId+"' ");
		hqlQuery.setQueryString(sql.toString());
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("1", 1);
		hqlQuery.setMap(map);
		hqlQuery.setClass1(TbConfigModelsEntity.class);
		...
		query.setMaxResults(hqlQuery.getPageSize());
	 	List list= query.list();
		List<TbConfigModelsEntity> volist = new ArrayList<TbConfigModelsEntity>();
		Object[]  obj = new Object[volist.size()];
		for (int i = 0; i < list.size(); i++) {
			obj = (Object[]) list.get(i);
			TbConfigModelsEntity entity=new TbConfigModelsEntity();
			TbQuotationsEntity quotationsEntity=new TbQuotationsEntity();
			quotationsEntity.setId(obj[0]==null?"":""+obj[0]);
			entity.setTbQuotations(quotationsEntity);
			entity.setId(obj[1]==null?"":""+obj[1]);
			entity.setTotalPrice(obj[2]==null?"":""+obj[2]);
			TbDataSourceEntityEntity dataSourceEntity=new TbDataSourceEntityEntity();
			dataSourceEntity.setId(obj[3]==null?"":""+obj[3]);
			entity.setTbDataSource(dataSourceEntity);
			entity.setName(obj[5]==null?"":""+obj[5]);
			volist.add(entity);
		}
		return new PageList(hqlQuery, volist, offset, curPageNO, allCounts);
	}*/
