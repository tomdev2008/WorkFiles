 ...
 * 
 * 策略相关操作 
 * 注意：策略0,1,2,3,4,5,13为系统保留,不允许设置删除等操作
 * 
...
public class Policy {
	...
	public static Policy getSingleInstance() {
		if (instance == null) {
			instance = new Policy();
		}
		return instance;
	...
	* IDRAC7:raw 0x2E 0xC2 0x57 0x01 0x00 0x00 0x00<br/>
	 * @param ser 服务器信息
	 * @param policyId 策略ID
	 * @param component 见Parameter.domainSystem domainCpu domainMemory
	 * @return PolicyEntity 策略实体
	 * @author zhangjh 新增日期：2012-8-20
	 * @since ipmi_sdk
	 */
	public PolicyEntity getPolicyById(SerInfo ser,int policyId,Integer component) {
	 ...
	 * @since ipmi_sdk
	 */
	public List<PolicyEntity> queryAllPolicy(SerInfo ser) {
		List<PolicyEntity> list = new ArrayList<PolicyEntity>();
		for (int i = 0; i < Parameter.maxPolicy; i++) {
			PolicyEntity entity = getPolicyById(ser,i,Parameter.domainSystem);
			if(entity!=null){
				list.add(entity);
				continue;
			}
			entity = getPolicyById(ser,i,Parameter.domainCpu);
			if(entity!=null){
				list.add(entity);
				continue;
			}
			entity = getPolicyById(ser,i,Parameter.domainMemory);
			if(entity!=null){
				list.add(entity);
			}
		}
	 ...
	 * @since ipmi_sdk
	 */
	public boolean setPolicyStatus(SerInfo ser,int policyId, boolean status,int component) {
		...
		if(StringUtils.equals(version, Parameter.NodeManager_15)){
			s_status = "0x01";
			if (!status)
				s_status = "0x00";
			raw = " raw 0x2e 0xc0 0x57 0x01 0x00 " + s_status + " "+domain+" 0x" + Util.int2Hex(policyId);
		}
		...
		PolicyEntity entity = getPolicyById(ser,policyId,component);
	 ...
	 * 删除策略<br/>
	 * 命令<br/>
	 * NM 1.5:raw 0x2e 0xc1 0x57 0x01 0x00 0x00 0x05 0x00 0x03 0xC8 0x00 0x70 0x17 0x00 0x00 0x00 0x00 0x01 0x00<br/>
	 * NM 2.0:raw 0x2e 0xc1 0x57 0x01 0x00 0x00 0x01 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00<br/>
	 * IDRAC7:raw 0x2e 0xc1 0x57 0x01 0x00 0x00 0x06 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00<br/>
	 * 
	 * @param policyId
	 * @return boolean
	 * @author zhangjh 新增日期：2012-8-20
	 * @since ipmi_sdk
	 */
	public boolean removePolicyID(SerInfo ser,int policyId,int component){
		...
		if(entity.isPolicyEnabled()){
			setPolicyStatus(ser,policyId, false,component);
		}
		String raw = "";
		String domain =Util.getDomain(component);
		String version = supportTool.getIpmiVersion(ser);
		if(StringUtils.equals(version, Parameter.NodeManager_15)){
			raw = " raw 0x2e 0xc1 0x57 0x01 0x00 "+domain+" 0x"+Util.int2Hex(policyId)+" 0x00 0x03 0xC8 0x00 0x70 0x17 0x00 0x00 0x00 0x00 0x01 0x00";
		}else if(StringUtils.equals(version, Parameter.NodeManager_20)){
			raw = " raw 0x2e 0xc1 0x57 0x01 0x00 "+domain+" 0x"+Util.int2Hex(policyId)+" 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00";
		}else if(StringUtils.equals(version, Parameter.NodeManager_Unkown)){
			raw = " raw 0x2e 0xc1 0x57 0x01 0x00 "+domain+" 0x"+Util.int2Hex(policyId)+" 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00";
		}else{
			Errors errors = new Errors(ErrorCode.NOT_SUPPORT_POLICY,"Host not support this policy,Please check your server with the version.Error host:"+ser.getHost());
		...
		entity = getPolicyById(ser,policyId,component);
		if(entity!=null){
	 ...
	 * @since ipmi_sdk
	 */
	public boolean setPolicy(SerInfo ser,PolicyEntity req){
		PolicyEntity entity = getPolicyById(ser,req.getPolicyId(),req.getComponent());

		if(entity!=null && entity.isPolicyEnabled()){
			setPolicyStatus(ser,req.getPolicyId(), false,entity.getComponent());
			log.info("******************************修改策略******************************");
		}else{
			log.info("******************************新增策略******************************");
		}
		...
		PolicyEntity cc = getPolicyById(ser,req.getPolicyId(),req.getComponent());
		if(cc==null)
			return false;
		if(req.isPolicyEnabled())
			setPolicyStatus(ser,req.getPolicyId(), true,req.getComponent());
	...
	* @since ipmi_sdk
	 */
	public void removeAllPolicy(SerInfo ser){
		List<PolicyEntity> list = queryAllPolicy(ser);
		for(int i=0;i<list.size();i++){
			PolicyEntity e = (PolicyEntity) list.get(i);
			if(e.getPolicyId()<=5||e.getPolicyId()==13)
				continue;
			removePolicyID(ser,e.getPolicyId(),e.getComponent());
		}
	}
