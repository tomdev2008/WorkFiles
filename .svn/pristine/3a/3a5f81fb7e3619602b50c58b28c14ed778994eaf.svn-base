...
public class CollectDataService {
	...
	public void collectMachineInfo (){
		...
		for(TbServer server :serverList){
			...
			//获取服务器的进风口温度的当前功耗
			Integer inletTemp = sensor.getTempSensorReading(ser,Parameter.InletTemp);
			if(inletTemp!=null){
				int a = inletTemp;
			...
			List<GlobalEntity>  aa = sensor.getSensorInfo(ser, Parameter.ExhaustTemp);
			cache.set(ser.getHost()+MemcachedKey.IPMI_EXHAUSTTEMP, aa);
			aa = sensor.getSensorInfo(ser, Parameter.InletTemp);
			cache.set(ser.getHost()+MemcachedKey.IPMI_INLETTTEMP, aa);
			List<List<GlobalEntity>> bb = sensor.getBatteryInfo(ser);
			cache.set(ser.getHost()+MemcachedKey.IPMI_BATTERY, bb);
			List<GlobalEntity> cc = sensor.getSensorInfo(ser, Parameter.FanRedundancy);
			cache.set(ser.getHost()+MemcachedKey.IPMI_FANREDUNDANCY, cc);
			List<List<GlobalEntity>> dd = sensor.getFanRPMInfo(ser);
			cache.set(ser.getHost()+MemcachedKey.IPMI_FANRPM, dd);
			dd = sensor.getCPUInfo(ser);
			cache.set(ser.getHost()+MemcachedKey.IPMI_CPU, dd);
			aa = sensor.getSensorInfo(ser, Parameter.PwrConsumption);
